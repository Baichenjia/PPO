...
def update_actor(buf_tensors, actor_critic, actor_optimizer):
    obs, act, adv, _, _ = buf_tensors
    with tf.GradientTape() as actor_tape:
        _, logp, _, _ = actor_critic.choose_action_prob(obs, act)
        pi_loss = -tf.reduce_mean(logp * adv)  # actor 损失
    # update
    actor_gradients = actor_tape.gradient(
        pi_loss, actor_critic.actor_network.trainable_variables+[actor_critic.log_std])
    actor_optimizer.apply_gradients(zip(
        actor_gradients, actor_critic.actor_network.trainable_variables+[actor_critic.log_std]))
    return pi_loss, logp


def update_critic(buf_tensors, actor_critic, critic_optimizer):
...
def vpg(env_fn, ac_kwargs=dict(),                  # ac_kwargs 存储了网络结构的参数
        seed=0, steps_per_epoch=4000, epochs=50,
        gamma=0.99, lam=0.97,                      # gamma, lambda 的设置
        pi_lr=3e-4, vf_lr=1e-3,                    # 学习率的设置
        train_v_iters=80, max_ep_len=1000, logger_kwargs=dict(), save_freq=10):
    ...
    for epoch in range(epochs):
        ...
        pi_loss, logp = update_actor(buf_tensors, actor_critic, actor_optimizer)
        # update critic multiple times
        v_loss, v = None, None
        for i in range(train_v_iters):
            v_loss, v = update_critic(buf_tensors, actor_critic, critic_optimizer)

        # loss after update, for record
        _, new_logp, _, _ = actor_critic.choose_action_prob(obs, act)
        new_pi_loss = -tf.reduce_mean(new_logp * adv)
        new_v = actor_critic.get_critic_output(obs)
        new_v_loss = tf.reduce_mean((ret - new_v) ** 2)  # critic 损失
        delta_loss_pi = new_pi_loss - pi_loss
        delta_loss_v = new_v_loss - v_loss

        # record to debug
        approx_kl = tf.reduce_mean(logp_old - new_logp)   # a sample estimate for KL-divergence, easy to compute
        ...
        print("Entropy: \t\t", approx_ent.numpy())
        print("kL: \t\t", approx_kl.numpy())
...
if __name__ == '__main__':
    ...
    # 训练
    # vpg(lambda: gym.make(args.env), ac_kwargs=dict(hidden_sizes=[args.hid] * args.l),
